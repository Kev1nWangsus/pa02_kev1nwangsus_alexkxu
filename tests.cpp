...
void test_moviebst_search() {
    string test1 = "search(): empty MovieBST";
    MovieBST mbst1;
    string s1 = "Jaws";
    assertEquals(mbst1.search(s1), false, test1);

    string test2 = "search(): empty string";
    string s2 = "";
    mbst1.insert(s2, 1.0);
    assertEquals(mbst1.search(s2), true, test2);

    string test3 = "search(): does not contain parameter";
    string s3 = "Green Book";
    assertEquals(mbst1.search(s3), false, test3);
}
